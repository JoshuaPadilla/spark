...
describe('SessionsService', () => {
  ...
  it('allows a user with one active port to start the other free port', async () => {
    userService.findById.mockResolvedValue({
      id: 'user-1',
      activePort: 1,
      balance: 25,
    ...
    const result = await service.startSession('user-1', 2, 5);

    expect(userService.deductBalance).toHaveBeenCalledWith('user-1', 5);
    expect(mqttService.sendNewSession).toHaveBeenCalledWith(2, 5 * 60 * 1000);
    expect(userService.markSessionActive).toHaveBeenCalledWith(
      'user-1',
      2,
      'Charging started on Port 2.',
    );
    expect(result).toEqual({
      success: true,
      pending: false,
      ...
      minutes: 5,
      cost: 5,
      message: 'Charging started on Port 2.',
    });
  });
  ...
  it('saves paused time immediately from the live status snapshot', async () => {
    ...
    const result = await service.pauseSession('user-1', 1);
  ...
  it('pauses the selected port when the user owns both ports', async () => {
    userService.findById.mockResolvedValue({
      activePort: 3,
      id: 'user-1',
    });
    ...
    const result = await service.pauseSession('user-1', 2);

    expect(mqttService.sendPause).toHaveBeenCalledWith(2);
    expect(userService.savePausedSession).toHaveBeenCalledWith(
      'user-1',
      2,
      119000,
      'Port 2 paused. 1:59 saved for resume.',
    );
    expect(result).toEqual({ success: true, port: 2, remainingMs: 119000 });
  });
});
